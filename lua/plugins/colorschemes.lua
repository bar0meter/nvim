...
schemes.tokyonight = function()
  ...
  for group, fg in pairs({
    ElNormal = palette.blue,
    ElInsert = palette.green,
    ElVisual = palette.orange,
    ElCommand = palette.yellow,
    ElReplace = palette.red,
    ElTerm = palette.purple,
  }) do
    vim.api.nvim_set_hl(0, group, { fg = fg, bg = bg, bold = true })
  end
  for _, link in ipairs({
    { "ElVisualLine", "ElVisual" },
    { "ElVisualBlock", "ElVisual" },
    { "ElSelect", "ElVisual" },
    { "ElVirtualReplace", "ElReplace" },
  }) do
...
return {
  ...
  setup = function()
    ...
    -- Shared: strip italics from whatever scheme is active
    -- (colorbuddy sets them under `cterm`).
