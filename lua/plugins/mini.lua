return {
  ...
  setup = function()
      ...
      content = {
        active = function()
          ...
          if devinfo ~= "" then
            left = {
              "%#", sep_mode, "#", RIGHT,
              "%#MiniStatuslineDevinfo# ", devinfo, " ",
              "%#MiniStatuslineSepBC#", RIGHT,
            }
          else
    ...
    -- only on these events, not on every redraw.
    vim.api.nvim_create_autocmd({ "BufEnter", "DirChanged", "FocusGained", "BufWritePost" }, {
      callback = function(ev)
        local name = vim.api.nvim_buf_get_name(ev.buf)
        local dir = name:match("^oil://(.*)") or (name ~= "" and vim.fn.fnamemodify(name, ":h")) or vim.fn.getcwd()
        if vim.fn.isdirectory(dir) == 0 then
          dir = vim.fn.getcwd()
        end
        local out = vim.fn.systemlist({ "git", "-C", dir, "rev-parse", "--abbrev-ref", "HEAD" })
        vim.b[ev.buf].airline_branch = (vim.v.shell_error == 0 and out[1]) or ""
      end,
    })
